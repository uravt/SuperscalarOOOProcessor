...
/* Load Binary. */
uint32_t load(char *bmk, Memory &memory, Processor &processor)
{
    Elf32_Ehdr ehdr;
    Elf32_Shdr shdr;
    ...
    for (int i = 0; i < ehdr.e_shnum; i++)
    {
        ...
        if ((shdr.sh_flags & SHF_EXECINSTR) != 0 && shdr.sh_addr == 0)
        { /* Text section -- we hardcoded this to zero during compilation. */
            ...
            for (int j = 0; j < (int)shdr.sh_size; j += 4)
            {
                ...
                if (num_read != 4)
                {
                    cout << "Could not populate memory from section: " << shdr.sh_addr << ": bytes read=" << j + num_read << ", section header size=" << shdr.sh_size << "\n";
                    return 0;
                }
                memory.access((uint32_t)shdr.sh_addr + j, dummy_word, word, false, true);
                processor.loadWordOOO((uint32_t)shdr.sh_addr + j, word);
            }
            fclose(binary_copy);
            return shdr.sh_size;
        }
...
int main(int argc, char *argv[])
{
    ...
    while (true)
    {
        ...
        case 'h':
            ...
            exit(0);
        case 'b':
            end_pc = load(optarg, memory, processor);
            break;
        case 'O':
            break;
        case '0':
    ...
    processor.setOOOOptLevel(optLevel);
